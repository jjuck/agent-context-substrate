...
`Hermes LLM Wiki Harness`는 Hermes 대화 세션을 장기적으로 재사용 가능한 지식 substrate로 바꾸는 도구입니다.
...
1. `wiki-harness` user plugin: session finalize hook과 `/harness`, `/packet`, `/wiki-resume`, `/wiki-lint` 명령을 제공합니다.
2. `wiki_harness` context engine: `wiki_recovery_context`, `wiki_knowledge_search`, `wiki_knowledge_expand` 도구를 제공합니다.
...
`install-plugin`은 `~/.hermes/plugins/wiki-harness/local_config.py`에 `<PROJECT_ROOT>`와 `<WIKI_ROOT>`를 기록합니다. 이 파일은 사용자별 local 설정이므로 public template의 hardcoded 경로와 분리됩니다.
...
| 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `HERMES_WIKI_HARNESS_PROJECT_ROOT` | 사용자 harness project path | harness package와 `data/` 위치 |
| `HERMES_WIKI_HARNESS_WIKI_ROOT` | 사용자 LLM Wiki vault path | Obsidian wiki root |
| `HERMES_WIKI_HARNESS_AUTO_FINALIZE` | `true` | session finalize 자동 처리 |
| `HERMES_WIKI_HARNESS_MIN_MESSAGE_COUNT` | `3` | 너무 짧은 session 제외 |
| `HERMES_WIKI_HARNESS_ALLOWED_SOURCES` | `telegram,cli` | 자동 처리할 raw session source |
| `HERMES_WIKI_HARNESS_GATEWAY_POLICY` | `trigger-only` | gateway source를 durable 대상에서 보수적으로 제외 |
| `HERMES_WIKI_HARNESS_PROMOTION_MODE` | `packet-only` | `packet-only` 또는 legacy `full` |
| `HERMES_WIKI_HARNESS_SKIP_TITLE_PATTERNS` | empty | 자동 처리 제외 title pattern |
...
`context.engine: wiki_harness`가 활성화되어 있으면 Hermes agent는 이전 지식이 필요할 때 read-only retrieval을 사용할 수 있습니다.
