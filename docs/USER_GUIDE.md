...
`Agent Context Substrate`는 Hermes와 Codex 대화 세션을 장기적으로 재사용 가능한 지식 substrate와 judge-guarded LLM Wiki로 바꾸는 도구입니다.
...
권장 LLM Wiki:
%USERPROFILE%\Documents\LLM Wiki (portable default template)
...
packaged Codex finalize가 성공하면 보통 아래 artifact가 생깁니다.
...
| `AGENT_CONTEXT_SUBSTRATE_WIKI_ROOT` | runtime override. `WIKI_PATH`, installed `local_config.json`, `%USERPROFILE%\Documents\LLM Wiki` template보다 우선 | Obsidian wiki root |
...
특정 Hermes session을 수동 finalize합니다. Hermes plugin 기본 설정에서는 `packet-only`로 처리합니다.
...
packaged Codex 자동화는 기본으로 rubric 기반 flexible 통합을 사용합니다. standalone `plan-wiki-patches` 명령은 명시적으로 바꾸지 않으면 기존 managed claim block proposal을 유지합니다.
...
아래 수동 명령들은 Obsidian을 수정하지 않습니다. 수동 실제 쓰기는 patch Markdown/JSON을 검토한 뒤 아래처럼 명시합니다.
...
standalone/Hermes packet workflow는 명시적으로 설정하지 않으면 Obsidian을 자동 수정하지 않습니다. Obsidian이 수정되는 경로:

- packaged Codex `apply-flexible` automation과 승인된 write-judge decision
...
Obsidian LLM Wiki:
%USERPROFILE%\Documents\LLM Wiki portable default template 또는 explicit <WIKI_ROOT>
